...
namespace samurai
{
	...
	struct StaticVector
	{
		...
		{
			SAMURAI_ASSERT(index < size_, "buffer overflow on acces");
			return (beg_)[index];
		}
		...
		{
			SAMURAI_ASSERT(size_ < MAX_SIZE, "exceded max size in push back");
			beg_[size_] = el;
			size_++;
		}
		...
		void pop_back()
		{
			SAMURAI_ASSERT(size_ > 0, "buffer underflow on pop back");
			size_--;
		}
