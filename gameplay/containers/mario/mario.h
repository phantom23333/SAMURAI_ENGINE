...
struct Mario: public Container
{
	...
	{
		size_t s = 0;
		if (requestedInfo.getFileSizeBinary(SAMURAI_RESOURCES_PATH "/mario/map1.scene", s))
		{
			if (s == mapSize.x * mapSize.y)
			{
				requestedInfo.readEntireFileBinary(SAMURAI_RESOURCES_PATH "/mario/map1.scene", map, mapSize.x * mapSize.y);
			}
			else { return 0; }
		}
		else { return 0; }
	...
	bool create(RequestedContainerInfo &requestedInfo, samurai::StaticString<256> commandLineArgument)
	{
		...
		marioTexture.loadFromFile(SAMURAI_RESOURCES_PATH "/mario/mario.png", true, false);
		marioTexture.loadFromFile(SAMURAI_RESOURCES_PATH "/mario/mario.png", true, false);
		...
		fileChanged.setFile(SAMURAI_RESOURCES_PATH "/mario/map1.scene");
		...
		RequestedContainerInfo &requestedInfo)
	{
...

		{
			...
			if (input.buttons[samurai::Button::Space].pressed() && player.grounded)
			{
				player.jump(75);
			}
			...
			//phisics
			{
				...
				player.applyGravity(250.f * input.deltaTime);
