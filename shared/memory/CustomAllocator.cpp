...
namespace memory
{
	...
	void CustomAllocator::init(void *baseMemory, size_t memorySize)
		{
			heapSize = memorySize; 
			...
			SAMURAI_ASSERT(memorySize >= 100, "Memory size must be at least 100 bytes");
	...
	void *CustomAllocator::allocate(size_t size)
		{
			...
			SAMURAI_ASSERT(baseMemory, "Allocator not initialized"); //err allocator not initialized
			...
			while (true)
			{
					...
					else //this is not the last free block 
					{
						...
						if (last == nullptr || last >= this->end) // this is the first free block but not the last 
						{
							...
							if (currentSize - aligned8Size < 24)
							{
								...
								{
									//heap corrupted
									SAMURAI_ASSERT(0, "heap corupted");
								}
				...
				else
				{
					if (current->next == nullptr || current->next >= this->end)
					{
						...
						{
							SAMURAI_ASSERT(0, "Allocator out of memory");
						}
			...
			SAMURAI_ASSERT(0, "unreachable code");
			return nullptr;
		}

	void CustomAllocator::free(void *mem)
		{
		...
		#pragma region check validity
			...
			SAMURAI_ASSERT(allocatedBLockHeader->guard == GUARD_VALUE, "invalid free or double free"); //invalid free or double free
			...
			else
			{
				...
				while (true)
				{
					...
					if (current == nullptr || current >= this->end)
					{
						...
						SAMURAI_ASSERT(0, "heap corupted or an invalid free");
	...
	//void *CustomAllocator::threadSafeAllocate(size_t size)
	//	{
	//		mu.lock();
	//
	...
	//	}
	//
	//void CustomAllocator::threadSafeFree(void *mem)
	//	{
	//		if (mem == nullptr) { return; }
	//
	//		mu.lock();
	...
	void CustomAllocator::calculateMemoryMetrics(size_t &availableMemory, size_t &biggestBlock, int &freeBlocks)
		{
